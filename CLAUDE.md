...
- **Agents are expensive — inline first.** Spawning a subagent costs ≥5k tokens in overhead before any work, and a codebase-sweep Explore agent can hit 60k+ on 22 tool uses. Try direct tools first: `Bash` (grep/find/cat), `Read`, `WebSearch` in main context. Only spawn an Agent when the task genuinely needs > ~10 tool uses OR would flood the main context with noise you don't need. Always include a scope constraint in every agent prompt ("report in under X words", "use no more than N searches", "quick" or "medium" breadth for Explore — never "very thorough" by default). Avoid running two heavy agents in parallel — 150k tokens can be burned in one message launch.
