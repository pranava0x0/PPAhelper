...
- **Don't build DOM by assigning markup to the `innerHTML` property — a PreToolUse hook blocks it, even for a static trusted string.** Injecting an inline SVG or any markup that way fails the edit outright (XSS guard), regardless of whether the content is a literal. Build nodes with `document.createElement` / `createElementNS` + `setAttribute` instead. This repo has a shared `makeIcon(paths, size)` helper in `app.js` for stroke-icon SVGs (theme toggle, search magnifier) — reuse it. Swap children with `replaceChildren(...)` / `append(...)`. (Note: the hook also matches that property name in prose/comments, so describe it, don't paste the assignment.)
